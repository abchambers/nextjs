...
function ClassForecastHub({ official, assignment, submissions, roster, canManage, onSave, onPublish }: { official: ClassroomOfficialForecast | null; assignment: ClassroomAssignment | null; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; canManage: boolean; onSave: (forecast: ClassForecastSnapshot) => void; onPublish: (forecast: ClassForecastSnapshot) => void }) {
  ...
  const consensus = assignment ? buildClassForecastSnapshot(assignment, submissions, roster) : null;
  ...
  return (
    <main className={`${radarExpanded ? "app radar-expanded" : "app"} desk-${activeWorkspace?.kind ?? "public"}`}>
      ...
      {activeSection === "classroom" && session && activeWorkspace?.kind === "classroom" && <section className="workspace-card classroom-workspace">
        ...
        {classroomHubTab === "outlook" && <ClassForecastHub official={classroomOfficialForecast} assignment={selectedClassroomAssignment} submissions={assignmentSubmissions} roster={academicRoster.filter((member) => member.role === "student")} canManage={canManageActiveClassroom} onSave={(forecast) => saveOfficialClassForecast(forecast)} onPublish={(forecast) => saveOfficialClassForecast(forecast, true)} />}
