    ...
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      ...
      <section className="dashboard-grid">
        <article className="radar-card">
          ...
          <div className="radar-controls" aria-label="Radar controls"><label>Overlay<select value={radarOverlay} onChange={(event) => setRadarOverlay(event.target.value as "reflectivity" | "none")}><option value="reflectivity">Composite reflectivity</option><option value="none">Base map only</option></select></label><label>Opacity <input type="range" min="20" max="100" value={radarOpacity} onChange={(event) => setRadarOpacity(Number(event.target.value))} /> <span>{radarOpacity}%</span></label><small>{radarLoop ? "The NOAA loop is an external animation. Timestamped play/pause controls will arrive with a frame-capable source." : "Interactive controls are ready for saved locations and future radar layers."}</small></div>
          <div className="radar">{radarLoop ? <img key={radarRefreshToken} className="radar-loop" src="https://radar.weather.gov/ridge/standard/KFFC_loop.gif" alt="Animated NOAA radar loop from Peachtree City radar covering Athens, Georgia" /> : <RadarMap opacity={radarOpacity / 100} showReflectivity={radarOverlay === "reflectivity"} refreshToken={radarRefreshToken} />}</div>
