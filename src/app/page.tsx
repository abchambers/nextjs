...
function ClassroomAssignmentStudio({ assignments, submissions, roster, selectedAssignmentId, canManage, onCreate, onOpenForecast, onOpenAssignment, message }: { assignments: ClassroomAssignment[]; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; selectedAssignmentId: string; canManage: boolean; onCreate: (fields: ClassroomAssignmentFields) => void; onOpenForecast: () => void; onOpenAssignment: (assignment: ClassroomAssignment) => void; message: string }) {
  const [title, setTitle] = useState("");
  ...
  return <section className="assignment-studio">
    <header className="section-heading"><div><p className="eyebrow">Assignment studio</p><h2>Practice forecasting, day by day</h2><p>Create a focused sequence, then let each student submit their private forecast and evidence.</p></div><button type="button" onClick={onOpenForecast}>Open forecast</button></header>
    ...
    <section className="assignment-rail"><header><p className="eyebrow">Assignments</p><h3>Choose a class activity</h3></header><div>{assignments.map((assignment) => <button type="button" key={assignment.id} className={assignment.id === selectedAssignmentId ? "active" : ""} onClick={() => onOpenAssignment(assignment)}><span><strong>{assignment.title}</strong><small>{assignmentDates(assignment).map(forecastTargetTitle).join(" · ")}</small></span><b>{assignment.status}</b></button>)}{!assignments.length && <p className="empty">No assignments have been created yet.</p>}</div></section>
  ...
  return (
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      ...
      {activeSection === "classroom" && session && activeWorkspace?.kind === "classroom" && <section className="workspace-card classroom-workspace">
        ...
        {reviewTarget?.classroomId === activeWorkspace.classroomId && <ClassroomReviewPanel target={reviewTarget!} runs={reviewRuns} selectedRun={selectedReviewRun} notes={reviewNotes} comment={reviewComment} manualScore={reviewManualScore} message={reviewMessage} onSelectRun={setSelectedReviewRunId} onCommentChange={setReviewComment} onManualScoreChange={setReviewManualScore} onSave={saveForecastReview} onClose={() => { setReviewTarget(null); setReviewRuns([]); setReviewNotes({}); setReviewComment(""); setReviewManualScore(""); }} />}
