...
function ClassForecastHubV2({ official, publicGuidance, canManage, onSave, onPublish }: { official: ClassroomOfficialForecast | null; publicGuidance: { date: string; label: string; high: number | null; low: number | null; shortForecast: string; precipitationChance: number | null }[]; canManage: boolean; onSave: (forecast: ClassForecastSnapshot) => void; onPublish: (forecast: ClassForecastSnapshot) => void }) {
  ...
  return <section className="class-forecast-hub class-forecast-hub-v2">
    <header className="section-heading"><div><p className="eyebrow">Class forecast</p><h2>Official class outlook</h2><p>A class-owned seven-day forecast, separate from local Frontline Forecast guidance.</p></div><span>{official?.published_at ? "Published to class" : "Classroom draft"}</span></header>
...
function ClassroomAssignmentStudio({ assignments, submissions, roster, selectedAssignmentId, canManage, onCreate, onSelectAssignment, message }: { assignments: ClassroomAssignment[]; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; selectedAssignmentId: string; canManage: boolean; onCreate: (fields: ClassroomAssignmentFields) => void; onSelectAssignment: (assignment: ClassroomAssignment) => void; message: string }) {
  const [title, setTitle] = useState("");
  ...
  return <section className="assignment-studio">
    <header className="section-heading"><div><p className="eyebrow">Assignments</p><h2>Class forecast exercises</h2><p>Create a dated exercise, then review each student’s submitted forecast.</p></div></header>
...
function ClassroomAssignmentDesk(props: { assignments: ClassroomAssignment[]; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; selectedAssignmentId: string; canManage: boolean; onCreate: (fields: ClassroomAssignmentFields) => void; onSelectAssignment: (assignment: ClassroomAssignment) => void; onSaveClassForecast: (snapshot: ClassForecastSnapshot) => void; onPublishClassForecast: () => void; message: string }) {
  ...
  return <><ClassroomAssignmentStudio assignments={props.assignments} submissions={props.submissions} roster={props.roster} selectedAssignmentId={props.selectedAssignmentId} canManage={props.canManage} onCreate={props.onCreate} onSelectAssignment={props.onSelectAssignment} message={props.message} />{assignment && snapshot && <ClassForecastOutlook assignment={assignment} snapshot={snapshot} canManage={props.canManage} onSave={() => props.onSaveClassForecast(snapshot)} onPublish={props.onPublishClassForecast} />}{props.canManage && assignment && <AssignmentAssessmentQueue assignment={assignment} submissions={[...latestByStudent.values()]} studentName={studentName} />}</>;
}
  ...
  return (
    <main className={`${radarExpanded ? "app radar-expanded" : "app"} desk-${activeWorkspace?.kind ?? "public"}`}>
      <header className="header">
        <div className="brand-lockup"><img src="/brand/frontline-forecast-mark.svg" alt="" /><div><p className="eyebrow">Human-first forecasting workspace</p><h1>Frontline Forecast</h1></div></div>
