...
function ClassroomAssignmentDesk({ assignments, selectedAssignmentId, canManage, onSelect, onCreate, onOpenForecast, message }: { assignments: ClassroomAssignment[]; selectedAssignmentId: string; canManage: boolean; onSelect: (id: string) => void; onCreate: (fields: Pick<ClassroomAssignment, "title" | "instructions" | "target_date" | "due_at" | "status">) => void; onOpenForecast: () => void; message: string }) {
...
export default function Home() {
  ...
  return (
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      ...
      {activeSection === "classroom" && session && activeWorkspace?.kind === "classroom" && <section className="workspace-card classroom-workspace"><div className="section-heading"><div><p className="eyebrow">School workspace</p><h2>{activeWorkspace.label}</h2><p>{canManageActiveClassroom ? "Create assignments, review class context, and move into the forecast workspace." : "Your class assignments and forecast work live here."}</p></div><span>{activeWorkspace.role ?? "member"}</span></div><ClassroomAssignmentDesk assignments={classroomAssignments} selectedAssignmentId={selectedClassroomAssignmentId} canManage={canManageActiveClassroom} onSelect={setSelectedClassroomAssignmentId} onCreate={createClassroomAssignment} onOpenForecast={() => setActiveSection("forecast")} message={assignmentMessage} /></section>}
