...
type WorkspaceSection = "dashboard" | "radar" | "forecast" | "verify" | "classroom" | "control";
type LiveWeather = {
  location: string;
...
type ClassroomAssignmentSubmission = { id: string; user_id: string; created_at: string; status: string; assignment_id: string; forecast_periods: { valid_date: string; period: "day" | "night"; forecast_data: PeriodDraft; forecast_verifications: { score_data: { automaticScore?: number | null } | null }[] }[] };
...
function buildClassForecastSnapshot(assignment: ClassroomAssignment, submissions: ClassroomAssignmentSubmission[], totalStudents: number): ClassForecastSnapshot {
...
function ClassroomAssignmentDesk({ assignments, submissions, roster, selectedAssignmentId, canManage, onCreate, onOpenForecast, onOpenAssignment, onSaveClassForecast, onPublishClassForecast, message }: { assignments: ClassroomAssignment[]; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; selectedAssignmentId: string; canManage: boolean; onCreate: (fields: Pick<ClassroomAssignment, "title" | "instructions" | "target_date" | "due_at" | "status">) => void; onOpenForecast: () => void; onOpenAssignment: (assignment: ClassroomAssignment) => void; onSaveClassForecast: (snapshot: ClassForecastSnapshot) => void; onPublishClassForecast: () => void; message: string }) {
  ...
  const students = roster.filter((member) => member.role === "student");
  const classForecast = selectedAssignment ? buildClassForecastSnapshot(selectedAssignment, submissions, students.length) : null;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch(`${supabaseUrl}/rest/v1/forecast_runs?select=id,user_id,created_at,status,assignment_id,forecast_periods(valid_date,period,forecast_data,forecast_verifications(score_data))&classroom_id=eq.${activeWorkspace.classroomId}&assignment_id=eq.${selectedClassroomAssignmentId}&status=neq.withdrawn&order=created_at.desc`, { headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}` } })
  ...
  }, [activeWorkspaceKey, selectedClassroomAssignmentId, session]);
  ...
  useEffect(() => {
    ...
    fetch(`${supabaseUrl}/rest/v1/forecast_runs?select=id,user_id,created_at,status,location_name,forecast_periods(id,valid_date,period,forecast_data,forecast_verifications(score_data))&user_id=eq.${reviewTarget.userId}${contextFilter}&status=neq.withdrawn&order=created_at.desc`, { headers })
      .then(async (response) => {
        ...
        const reviewResponse = await fetch(`${supabaseUrl}/rest/v1/forecast_reviews?select=id,run_id,reviewer_id,comment,manual_score,created_at&run_id=in.(${runs.map((run) => run.id).join(",")})&order=created_at.desc`, { headers });
  ...
  async function saveForecastReview(runId: string) {
    ...
    if (!reviewComment.trim() && manualScore === null) { setReviewMessage("Add a comment or a manual score before saving."); return; }
    ...
    const response = await fetch(`${supabaseUrl}/rest/v1/forecast_reviews`, { method: "POST", headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify({ run_id: runId, reviewer_id: session.user.id, comment: reviewComment.trim() || null, manual_score: manualScore }) });
    const rows = await response.json().catch(() => []);
    ...
    setReviewComment(""); setReviewManualScore(""); setReviewMessage("Review saved.");
  }
  ...
  return (
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      ...
      {activeSection === "classroom" && session && activeWorkspace?.kind === "classroom" && <section className="workspace-card classroom-workspace"><div className="section-heading"><div><p className="eyebrow">School workspace</p><h2>{activeWorkspace.label}</h2><p>{canManageActiveClassroom ? "Create assignments, review class context, and move into the forecast workspace." : "Your class assignments and forecast work live here."}</p></div><span>{activeWorkspace.role ?? "member"}</span></div><ClassroomAssignmentDesk assignments={classroomAssignments} submissions={assignmentSubmissions} roster={academicRoster} selectedAssignmentId={selectedClassroomAssignmentId} canManage={canManageActiveClassroom} onCreate={createClassroomAssignment} onOpenForecast={() => setActiveSection("forecast")} onOpenAssignment={openClassroomAssignment} onSaveClassForecast={(snapshot) => saveClassForecastSnapshot(snapshot)} onPublishClassForecast={() => { const assignment = classroomAssignments.find((item) => item.id === selectedClassroomAssignmentId); if (!assignment) return; saveClassForecastSnapshot(buildClassForecastSnapshot(assignment, assignmentSubmissions, academicRoster.filter((member) => member.role === "student").length), true); }} message={assignmentMessage} /></section>}
