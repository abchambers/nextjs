...
function archiveVersionTitle(archive: SavedForecast) {
  ...
  return `${archiveTitle(archive)} · V${archive.versionNumber ?? 1} · ${status}`;
}
  ...
  return (
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      ...
      {activeSection === "verify" && session && <section className="workspace-card">
        {selectedArchive ? <><div className="section-heading"><div><h2>{archiveVersionTitle(selectedArchive)}</h2><p>Athens, GA · target: {selectedArchive.targetDate ?? "not set"}</p></div><div className="verification-score"><strong>{selectedAutomaticVerification?.dayScore !== null && selectedAutomaticVerification?.dayScore !== undefined ? `${selectedAutomaticVerification.dayScore}%` : selectedArchive.status === "draft" ? "Draft" : "Pending"}</strong><span>{selectedAutomaticVerification ? "automatic day score" : selectedArchive.status === "draft" ? "not graded" : "verification"}</span><button type="button" onClick={() => collectActuals(selectedArchive)}>Collect actuals</button></div></div>{verificationMessage && <p className="empty">{verificationMessage}</p>}
