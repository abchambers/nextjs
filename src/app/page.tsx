    ...
    try {
      ...
      if (session && supabaseUrl && supabaseKey && archive.periodIds?.day && archive.periodIds?.night) {
        ...
        const saved = await Promise.all([
          fetch(`${supabaseUrl}/rest/v1/forecast_verifications`, { method: "POST", headers, body: JSON.stringify({ forecast_period_id: archive.periodIds.day, observed_data: data.day, score_data: { automaticScore: verification.dayScore, method: "temperature (70) + precipitation occurrence (30)" } }) }),
          fetch(`${supabaseUrl}/rest/v1/forecast_verifications`, { method: "POST", headers, body: JSON.stringify({ forecast_period_id: archive.periodIds.night, observed_data: data.night, score_data: { automaticScore: verification.nightScore, method: "temperature (70) + precipitation occurrence (30)" } }) }),
        ]);
  ...
  return (
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      ...
      {activeSection === "forecast" && session && <section className="workspace-card">
        ...
        <section className="archive-summary"><div><strong>{archives.length} saved forecast{archives.length === 1 ? "" : "s"}</strong><span>Each save is a separate version with its own weather evidence.</span></div>{archives.length > 0 && <div className="archive-list">{archives.slice(0, 4).map((archive) => <button type="button" key={archive.id} onClick={() => { setSelectedArchiveId(archive.id); setActiveSection("verify"); }}>Forecast: {forecastTargetTitle(archive.targetDate)}<small>{archiveSubmissionTitle(archive)}</small></button>)}</div>}</section>
        <form id="forecast-form" onSubmit={saveForecast}>
      ...
      {activeSection === "verify" && session && <section className="workspace-card">
        {selectedArchive ? <><div className="section-heading"><div><h2>{archiveVersionTitle(selectedArchive)}</h2><p>Athens, GA · {archiveSubmissionTitle(selectedArchive)}</p></div><div className="verification-score"><strong>{selectedArchive.status === "draft" ? "Draft" : selectedAutomaticVerification?.day.complete && selectedAutomaticVerification?.night.complete ? "Verified" : "Pending"}</strong><span>{selectedArchive.status === "draft" ? "not graded" : "automatic verification"}</span><button type="button" onClick={() => collectActuals(selectedArchive)}>Collect actuals</button></div></div>{verificationMessage && <p className="empty">{verificationMessage}</p>}<div className="grade-summary"><div><span>Day · 7 AM–7 PM</span><strong>{scoreLabel(selectedAutomaticVerification?.dayScore, selectedAutomaticVerification?.day)}</strong><small>{selectedAutomaticVerification ? temperatureErrorLabel(selectedArchive.day.high, selectedAutomaticVerification.day, true) : "Collect observations after the period."}</small></div><div><span>Night · 7 PM–7 AM</span><strong>{scoreLabel(selectedAutomaticVerification?.nightScore, selectedAutomaticVerification?.night)}</strong><small>{selectedAutomaticVerification ? temperatureErrorLabel(selectedArchive.night.low, selectedAutomaticVerification.night, false) : "Collect observations after the period."}</small></div><div><span>Automatic rubric</span><strong>100 points</strong><small>Temperature accuracy 70 · precipitation occurrence 30</small></div></div>
