...
type RadarMapView = "composite" | "satellite" | "base" | "precipitation_new" | "clouds_new" | "pressure_new" | "wind_new" | "temp_new";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (settings.radarMapView && ["composite", "satellite", "base", "precipitation_new", "clouds_new", "pressure_new", "wind_new", "temp_new"].includes(settings.radarMapView)) setRadarMapView(settings.radarMapView);
    ...
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      ...
      <section className="dashboard-grid">
        <article className="radar-card">
          <div className="card-heading"><div><h2>{radarMapView === "satellite" ? "Satellite" : "Radar"}</h2><p>{radarMapView === "composite" ? `Live composite reflectivity · centered on ${selectedLocation.name}` : radarMapView === "satellite" ? `GOES-East GeoColor · current CONUS picture for ${selectedLocation.name}` : `OpenWeather map layer · centered on ${selectedLocation.name}`}</p></div><div className="actions">{radarMapView === "composite" && <button onClick={() => { setRadarLoop((value) => !value); setRadarPlaying(false); }}>{radarLoop ? "Interactive map" : "Radar timeline"}</button>}<button onClick={() => setRadarRefreshToken((value) => value + 1)}>Refresh</button><button onClick={() => setRadarExpanded((value) => !value)}>{radarExpanded ? "Exit expanded view" : "Expand radar"}</button></div></div>
          <div className="radar"><details className="radar-tools"><summary aria-label="Open radar controls">☰</summary><div><label>Map view<select value={radarMapView} onChange={(event) => { const view = event.target.value as RadarMapView; setRadarMapView(view); window.dispatchEvent(new CustomEvent("weather-desk-radar-layer", { detail: view === "composite" || view === "base" || view === "satellite" ? "none" : view })); if (view !== "composite") setRadarLoop(false); event.currentTarget.closest("details")?.removeAttribute("open"); }}><option value="composite">Composite reflectivity</option><option value="satellite">GOES-East satellite</option><option value="precipitation_new">Precipitation</option><option value="clouds_new">Cloud cover</option><option value="pressure_new">Pressure</option><option value="wind_new">Wind speed</option><option value="temp_new">Temperature</option><option value="base">Base map only</option></select></label>{radarMapView !== "satellite" && <><label className="alert-overlay-toggle"><input type="checkbox" checked={showNwsAlerts} onChange={(event) => { setShowNwsAlerts(event.target.checked); window.dispatchEvent(new CustomEvent("weather-desk-alert-overlay", { detail: event.target.checked })); event.currentTarget.closest("details")?.removeAttribute("open"); }} /> NWS watches &amp; warnings</label><label>Opacity <input type="range" min="20" max="100" value={radarOpacity} onChange={(event) => setRadarOpacity(Number(event.target.value))} /> <span>{radarOpacity}%</span></label></>}<small>{radarMapView === "satellite" ? "Official NOAA GOES-East GeoColor · visible by day and infrared overnight" : radarMapView === "composite" ? (radarLoop ? radarTimelineStatus : `NOAA composite reflectivity · ${selectedLocation.radarSite}`) : "OpenWeather weather-map layer · exclusive view"}</small></div></details>{radarLoop && radarMapView === "composite" && <div className="radar-playback"><button type="button" aria-label="Previous radar frame" disabled={!radarFrames.length} onClick={() => { setRadarPlaying(false); setRadarFrameIndex((index) => Math.max(0, index - 1)); }}>‹</button><button type="button" disabled={radarFrames.length < 2} onClick={() => setRadarPlaying((playing) => !playing)}>{radarPlaying ? "Pause" : "Play"}</button><button type="button" aria-label="Next radar frame" disabled={!radarFrames.length} onClick={() => { setRadarPlaying(false); setRadarFrameIndex((index) => Math.min(radarFrames.length - 1, index + 1)); }}>›</button><span>{radarFrameTime}</span></div>}{radarMapView === "satellite" ? <figure className="satellite-view"><img src={`https://cdn.star.nesdis.noaa.gov/GOES19/ABI/CONUS/GEOCOLOR/1250x750.jpg?refresh=${radarRefreshToken}`} alt="Latest NOAA GOES-East GeoColor image for the continental United States" /><figcaption>GOES-East GeoColor · {selectedLocation.name} is within this regional view</figcaption></figure> : <RadarMap location={selectedLocation} opacity={radarOpacity / 100} showReflectivity={radarMapView === "composite"} refreshToken={radarRefreshToken} timelineTileUrl={radarLoop && radarMapView === "composite" ? radarFrame?.tileUrl : null} />}</div>
