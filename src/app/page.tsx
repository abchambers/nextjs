...
type ClassroomHubTab = "overview" | "today" | "assignments" | "progress" | "settings";
type LiveWeather = {
  location: string;
...
type ClassForecastSnapshot = { generated_at: string; target_date: string; submitted_count: number; total_students: number; day: { high_f: number | null; pop: number | null; conditions: string[] }; night: { low_f: number | null; pop: number | null; conditions: string[] } };
type ClassroomAssignment = { id: string; classroom_id: string; title: string; instructions: string | null; target_date: string; due_at: string | null; status: "draft" | "open" | "closed"; instructor_forecast: InstructorForecastSnapshot | null; instructor_forecast_updated_at: string | null; class_forecast: ClassForecastSnapshot | null; class_forecast_updated_at: string | null; class_forecast_published_at: string | null; created_at: string };
...
function buildClassForecastSnapshot(assignment: ClassroomAssignment, submissions: ClassroomAssignmentSubmission[], students: AcademicRosterMember[]): ClassForecastSnapshot {
  ...
  const periodData = (period: "day" | "night") => latest.map((submission) => submission.forecast_periods.find((entry) => entry.period === period && entry.valid_date === assignment.target_date)?.forecast_data).filter((data): data is PeriodDraft => Boolean(data));
  ...
  return { generated_at: new Date().toISOString(), target_date: assignment.target_date, submitted_count: latest.length, total_students: studentIds.size, day: { high_f: averageForecastValue(day.map((period) => period.highLow)), pop: averageForecastValue(day.map((period) => period.rainChance)), conditions: commonConditions(day) }, night: { low_f: averageForecastValue(night.map((period) => period.highLow)), pop: averageForecastValue(night.map((period) => period.rainChance)), conditions: commonConditions(night) } };
}
...
function SchoolDesk({ workspace, classrooms, onOpenClassroom }: { workspace: WorkspaceContext; classrooms: WorkspaceContext[]; onOpenClassroom: (workspace: WorkspaceContext) => void }) {
  ...
  return <section className="school-desk"><header className="section-heading"><div><p className="eyebrow">School workspace</p><h2>{workspaceDeskLabel(workspace)}</h2><p>A private hub for the school’s classes, people, and teaching workflow. Nothing here feeds the public Weather Desk.</p></div><span>School only</span></header><div className="school-status-grid"><article><span>Classes</span><strong>{classrooms.length}</strong><small>available to this account</small></article><article><span>Teaching access</span><strong>{instructionalClasses.length}</strong><small>classes you can manage or review</small></article><article><span>Public output</span><strong>Off</strong><small>school forecasts remain internal</small></article></div><section className="school-class-directory"><header><div><p className="eyebrow">Classes</p><h3>Open a class desk</h3><p>Each class keeps its own assignments, submitted forecasts, feedback, and class-only forecast.</p></div></header><div>{classrooms.map((classroom) => <button type="button" key={classroom.key} onClick={() => onOpenClassroom(classroom)}><span><strong>{workspaceDeskLabel(classroom)}</strong><small>{classroom.detail}</small></span><em>{classroom.role ?? "member"}</em><b>Open</b></button>)}{!classrooms.length && <p className="empty">No classes are assigned to this school workspace yet.</p>}</div></section></section>;
}
...
function ClassroomProgress({ assignment, submissions, roster, canManage }: { assignment: ClassroomAssignment | null; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; canManage: boolean }) {
  const students = roster.filter((member) => member.role === "student");
  const studentIds = new Set(students.map((member) => member.userId));
  const studentCount = students.length;
  const uniqueSubmissions = new Set(submissions.filter((submission) => !canManage || studentIds.has(submission.user_id)).map((submission) => submission.user_id)).size;
  const snapshot = assignment ? buildClassForecastSnapshot(assignment, submissions, students) : null;
  return <section className="classroom-progress"><header><p className="eyebrow">Progress</p><h3>{canManage ? "Assignment progress" : "Your assignment progress"}</h3><p>{canManage ? "Completion and consensus update from the latest submitted forecast per student." : "Your forecast history and automated scores stay private in Verify."}</p></header>{assignment && <div className="classroom-progress-grid"><article><span>Completion</span><strong>{canManage ? `${uniqueSubmissions}/${studentCount || "—"}` : uniqueSubmissions ? "Complete" : "In progress"}</strong><small>{canManage ? "students with a submission" : "submission status"}</small></article>{canManage && snapshot && <><article><span>Day consensus</span><strong>{snapshot.day.high_f ?? "—"}° · {snapshot.day.pop ?? "—"}%</strong><small>{snapshot.day.conditions.join(" · ") || "No conditions yet"}</small></article><article><span>Night consensus</span><strong>{snapshot.night.low_f ?? "—"}° · {snapshot.night.pop ?? "—"}%</strong><small>{snapshot.night.conditions.join(" · ") || "No conditions yet"}</small></article></>}</div>}{!assignment && <p className="empty">Select an assignment to see its progress.</p>}</section>;
}
...
function ClassroomInstructorOverview({ assignment, submissions, roster, onReviewStudent }: { assignment: ClassroomAssignment | null; submissions: ClassroomAssignmentSubmission[]; roster: AcademicRosterMember[]; onReviewStudent: (student: AcademicRosterMember) => void }) {
  ...
  const rows = students.map((student) => {
    ...
    const reviews = submission?.forecast_reviews ?? [];
...
export default function Home() {
  ...
  const [classroomHubTab, setClassroomHubTab] = useState<ClassroomHubTab>("today");
  ...
  const canManageActiveClassroom = Boolean(session && activeWorkspace?.kind === "classroom" && (hasControlAccess || ["instructor", "assistant"].includes(activeWorkspace.role ?? "")));
  ...
  const openClassroomAssignment = (assignment: ClassroomAssignment) => {
    ...
    if (existingIndex >= 0) setSelectedForecastDay(existingIndex);
      ...
      const days = [...forecastRun.days, createForecastDay(assignment.target_date)].sort((a, b) => a.date.localeCompare(b.date));
      setForecastRun({ ...forecastRun, days });
      setSelectedForecastDay(days.findIndex((day) => day.date === assignment.target_date));
    }
    setAssignmentMessage(`Opened “${assignment.title}” for ${forecastTargetTitle(assignment.target_date)}. Your existing draft was kept.`);
  ...
  useEffect(() => {
    ...
    fetch(`${supabaseUrl}/rest/v1/classroom_assignments?select=id,classroom_id,title,instructions,target_date,due_at,status,instructor_forecast,instructor_forecast_updated_at,class_forecast,class_forecast_updated_at,class_forecast_published_at,created_at&classroom_id=eq.${activeWorkspace.classroomId}&order=target_date.asc`, { headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}` } })
  ...
  useEffect(() => {
    if (!session || !supabaseUrl || !supabaseKey || activeWorkspace?.kind !== "classroom" || !activeWorkspace.classroomId || !selectedClassroomAssignmentId) { setAssignmentSubmissions([]); return; }
    fetch(`${supabaseUrl}/rest/v1/forecast_runs?select=id,user_id,created_at,status,assignment_id,forecast_periods(valid_date,period,forecast_data,forecast_verifications(score_data)),forecast_reviews(id,run_id,reviewer_id,comment,manual_score,rubric_scores,created_at)&classroom_id=eq.${activeWorkspace.classroomId}&assignment_id=eq.${selectedClassroomAssignmentId}&status=neq.withdrawn&order=created_at.desc`, { headers: { apikey: supabaseKey, Authorization: `Bearer ${session.access_token}` } })
  ...
  }, [activeWorkspaceKey, assignmentSubmissionRefreshToken, selectedClassroomAssignmentId, session]);
  ...
  async function saveForecast(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (selectedClassroomAssignment && !forecastRun.days.some((day) => day.date === selectedClassroomAssignment.target_date)) {
      setSaveMessage(`This worksheet does not include ${forecastTargetTitle(selectedClassroomAssignment.target_date)}, the target date for “${selectedClassroomAssignment.title}.” Add or change that forecast day before submitting.`);
      return;
    }
  ...
  async function createClassroomAssignment(fields: Pick<ClassroomAssignment, "title" | "instructions" | "target_date" | "due_at" | "status">) {
    ...
    <main className={radarExpanded ? "app radar-expanded" : "app"}>
      <header className="header">
        ...
        <div className="header-meta">
          {session && <div className="workspace-menu-wrap"><button type="button" className="workspace-trigger" aria-expanded={workspaceMenuOpen} onClick={() => setWorkspaceMenuOpen((open) => !open)}><strong>{workspaceDeskLabel(activeWorkspace)}</strong><i aria-hidden="true">⌄</i></button>{workspaceMenuOpen && <div className="workspace-menu"><strong>Available workspaces</strong><small>Switch the forecasts and records you are working in. Your location stays available across workspaces.</small><div>{workspaceContexts.map((workspace) => <button type="button" key={workspace.key} className={workspace.key === activeWorkspaceKey ? "active" : ""} onClick={() => { setActiveWorkspaceKey(workspace.key); setWorkspaceMenuOpen(false); setWorkspaceContextStatus(`${workspaceDeskLabel(workspace)} is now active.`); }}><strong>{workspaceDeskLabel(workspace)}</strong><span>{workspace.detail}{workspace.role ? ` · ${workspace.role}` : ""}</span></button>)}</div>{workspaceContextStatus && <em>{workspaceContextStatus}</em>}</div>}</div>}
      ...
      <nav aria-label="Main navigation" className="navigation">
        ...
        {session && activeWorkspace?.kind === "organization" && <button className={activeSection === "school" ? "active" : ""} onClick={() => setActiveSection("school")}>School desk</button>}
        ...
        {hasAcademicReviewAccess && <button className={activeSection === "control" ? "active" : ""} onClick={() => setActiveSection("control")}>{hasControlAccess ? "Control center" : "Review desk"}</button>}
      </nav>
      ...
      {activeSection === "school" && session && activeWorkspace?.kind === "organization" && <section className="workspace-card"><SchoolDesk workspace={activeWorkspace} classrooms={workspaceContexts.filter((workspace) => workspace.kind === "classroom" && workspace.organizationId === activeWorkspace.organizationId)} onOpenClassroom={(workspace) => { setActiveWorkspaceKey(workspace.key); setClassroomHubTab("today"); setActiveSection("classroom"); }} /></section>}
      {activeSection === "classroom" && session && activeWorkspace?.kind === "classroom" && <section className="workspace-card classroom-workspace">
        <div className="section-heading"><div><p className="eyebrow">School workspace</p><h2>{activeWorkspace.label}</h2><p>{canManageActiveClassroom ? "Create assignments, review class context, and move into the forecast workspace." : "Your class assignments and forecast work live here."}</p></div><span>{activeWorkspace.role ?? "member"}</span></div>
        <nav className="classroom-hub-tabs" aria-label="Classroom sections">
          {canManageActiveClassroom && <button type="button" className={classroomHubTab === "overview" ? "active" : ""} onClick={() => setClassroomHubTab("overview")}>Overview</button>}
          <button type="button" className={classroomHubTab === "today" ? "active" : ""} onClick={() => setClassroomHubTab("today")}>Today</button>
        ...
        {classroomHubTab === "overview" && canManageActiveClassroom && <ClassroomInstructorOverview assignment={selectedClassroomAssignment} submissions={assignmentSubmissions} roster={academicRoster} onReviewStudent={(student) => { setReviewTarget(student); setActiveSection("control"); }} />}
        ...
        {classroomHubTab === "settings" && canManageActiveClassroom && <section className="classroom-settings"><header><p className="eyebrow">Class settings</p><h3>Teaching controls</h3><p>Roster and access remain controlled in the Review Desk / Control Center. Assignment and class-forecast publications stay internal to this class.</p></header><div><article><strong>Privacy</strong><span>Student forecasts: student + teaching team only</span></article><article><strong>Class publication</strong><span>Class consensus: class members only</span></article><article><strong>Public Weather Desk</strong><span>Not connected to school forecasts</span></article></div><button type="button" onClick={() => setActiveSection("control")}>Open review desk</button></section>}
      </section>}
      {activeSection === "control" && session && hasControlAccess && <section className="workspace-card control-center"><div className="section-heading"><div><p className="eyebrow">Workspace administration</p><h2>Control center</h2><p>Defaults, delivery status, and access controls in one place.</p></div><span>{role === "owner" ? "Owner" : "Admin"}</span></div><div className="control-status-grid"><article><span>Forecasting</span><strong>{defaultForecastDays}-day default</strong><small>New worksheets begin with this horizon.</small></article><article><span>Verification</span><strong>Automated</strong><small>Daily actuals and preliminary scores are scheduled server-side.</small></article><article><span>Archive</span><strong>Auditable</strong><small>Submitted forecasts retain their evidence and revision history.</small></article></div><div className="control-layout"><section className="control-panel"><header><p className="eyebrow">Workspace</p><h3>Defaults</h3><p>Saved on this browser so your desk opens the way you work.</p></header><div className="settings-grid"><label>Default location<select value={defaultLocationId} onChange={(event) => setDefaultLocationId(weatherDeskLocation(event.target.value).id)}>{weatherDeskLocations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select><small>Used when no workspace location has been saved.</small></label><label>New forecast horizon<select value={defaultForecastDays} onChange={(event) => setDefaultForecastDays(Number(event.target.value) as 1 | 3 | 7)}><option value={1}>1 day</option><option value={3}>3 days</option><option value={7}>7 days</option></select><small>A clean worksheet after submission uses this number of days.</small></label><label>Appearance<select value={theme} onChange={(event) => setTheme(event.target.value as "light" | "dark")}><option value="light">Light mode</option><option value="dark">Dark mode</option></select><small>Applies to this browser immediately.</small></label><label>Default map view<select value={radarMapView} onChange={(event) => selectRadarView(event.target.value as RadarMapView)}><option value="composite">Composite reflectivity</option><option value="satellite">GOES-East satellite</option><option value="precipitation_new">Precipitation</option><option value="clouds_new">Cloud cover</option><option value="pressure_new">Pressure</option><option value="wind_new">Wind speed</option><option value="temp_new">Temperature</option><option value="ndfd_maxt">NWS maximum temperature map</option><option value="ndfd_pop12">NWS precipitation chance map</option><option value="ndfd_windspd">NWS wind speed map</option><option value="base">Base map only</option></select><small>Also updates the live dashboard map.</small></label><label>Radar opacity<input type="range" min="20" max="100" value={radarOpacity} onChange={(event) => setRadarOpacity(Number(event.target.value))} /><strong>{radarOpacity}%</strong><small>Applies to weather-map overlays.</small></label><label className="setting-check"><input type="checkbox" checked={showNwsAlerts} onChange={(event) => { setShowNwsAlerts(event.target.checked); window.dispatchEvent(new CustomEvent("weather-desk-alert-overlay", { detail: event.target.checked })); }} /><span><strong>Show NWS alerts</strong><small>Display active watches, warnings, and advisories on the map.</small></span></label></div><div className="settings-actions"><button type="button" onClick={() => { setLocationId(defaultLocationId); setActiveSection("dashboard"); setControlMessage("Opened your default workspace."); }}>Open default workspace</button><button type="button" onClick={() => { setDefaultLocationId(defaultWeatherDeskLocation.id); setDefaultForecastDays(1); selectRadarView("composite"); setRadarOpacity(72); setShowNwsAlerts(true); setTheme("light"); setControlMessage("Desk defaults restored for this browser."); }}>Restore desk defaults</button></div>{controlMessage && <p className="control-message" role="status">{controlMessage}</p>}</section><aside className="control-panel control-delivery"><header><p className="eyebrow">Operations</p><h3>Delivery and data</h3><p>Production services continue even when this local computer is off.</p></header><div className="control-service-list"><div><span>Public workspace</span><strong>Vercel production</strong><small>Independent of your local Terminal.</small></div><div><span>Account archive</span><strong>{supabaseUrl && supabaseKey ? "Supabase configured" : "Configuration missing"}</strong><small>Auth and cloud forecast records are separate from this device.</small></div><div><span>Live weather sources</span><strong>{liveWeather ? "Responding" : "Checking"}</strong><small>NWS, NOAA, and model sources load when a workspace is opened.</small></div><div><span>Local preferences</span><strong>This browser</strong><small>Theme, map defaults, and draft behavior stay device-specific.</small></div></div></aside></div><PeopleDirectory profiles={profiles} onRoleChange={setProfileRole} onProfileSave={saveProfileDetails} message={profileMessage} /></section>}
